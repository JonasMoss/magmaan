...
FitError post_to_fit(PostError e) {
  return fit_err(FitError::Kind::NumericIssue, std::move(e.detail));
}

FitError model_to_fit(ModelError e, const char* who) {
  return fit_err(FitError::Kind::NonPositiveDefiniteSigma,
...
enum class LiftCovarianceKind { Theta, Psi };

struct LiftLink {
  Eigen::Index full_vech_row = 0;
  Eigen::Index active_row = -1;
  Eigen::Index active_col = -1;

  bool lifted() const noexcept {
    return active_row >= 0 && active_col >= 0;
  }
};

struct LiftBlock {
  std::size_t block = 0;
  LiftCovarianceKind kind = LiftCovarianceKind::Theta;
  Eigen::Index dim = 0;
  std::vector<Eigen::Index> active;
  Eigen::VectorXd c0;
  Eigen::MatrixXd D;
  Eigen::Index lift_offset = 0;
  std::vector<LiftLink> links;

  Eigen::Index active_dim() const noexcept {
    return static_cast<Eigen::Index>(active.size());
  }
  Eigen::Index n_lift() const noexcept {
    const Eigen::Index q = active_dim();
    return detail::vech_len(q);
  }
};

struct PsdLiftLayout {
  Eigen::Index n_alpha = 0;
  Eigen::Index n_lift = 0;
  Eigen::Index n_link = 0;
  std::vector<LiftBlock> blocks;
};

Eigen::MatrixXd lower_from_x(const Eigen::VectorXd& x, Eigen::Index offset,
                             Eigen::Index q) {
  Eigen::MatrixXd L = Eigen::MatrixXd::Zero(q, q);
  Eigen::Index k = offset;
...
build_psd_lift_layout(const spec::LatentStructure& pt,
  ...
  constexpr double structural_tol = 1e-12;
  PsdLiftLayout out;
  ...
  for (std::size_t b = 0; b < rep.dims.size(); ++b) {
    for (const LiftCovarianceKind kind :
      ...
      const Eigen::Index dim =
          kind == LiftCovarianceKind::Theta
              ? rep.dims[b].n_observed
              : rep.dims[b].n_latent;
      if (dim == 0) continue;
      LiftBlock block;
      block.block = b;
  ...
  // Primitive covariance cells are affine in the original partable
  // parameters. Build that map directly from MatrixRep instead of asking the
  // full evaluator to assemble arbitrary basis points: a basis point in B can
  // make I-B singular even though Θ/Ψ themselves are perfectly well defined.
  for (std::size_t i = 0; i < pt.size(); ++i) {
    const model::Cell& cell = rep.cell_for_row[i];
    if (!cell.used ||
        (cell.mat != model::MatId::Theta &&
         cell.mat != model::MatId::Psi)) {
      continue;
    }
    const std::size_t b = static_cast<std::size_t>(cell.block);
    ...
    if (pt.free[i] > 0) {
      ...
      block.c0(row) = con.theta0(k);
      if (con.n_alpha > 0) block.D.row(row) = con.K().row(k);
    } else {
      block.c0(row) =
...
model::CovarianceOverrides
covariance_overrides_from_x(const model::MatrixRep& rep,
                            const PsdLiftLayout& layout,
                            const Eigen::VectorXd& x) {
  model::CovarianceOverrides out;
  out.theta.reserve(rep.dims.size());
  out.psi.reserve(rep.dims.size());
  for (const auto& dims : rep.dims) {
    out.theta.push_back(Eigen::MatrixXd::Zero(
        dims.n_observed, dims.n_observed));
    out.psi.push_back(Eigen::MatrixXd::Zero(
        dims.n_latent, dims.n_latent));
  }
  for (const auto& block : layout.blocks) {
    ...
    Eigen::MatrixXd& target =
        block.kind == LiftCovarianceKind::Theta
            ? out.theta[block.block]
            : out.psi[block.block];
    for (Eigen::Index j = 0; j < q; ++j) {
      for (Eigen::Index i = 0; i < q; ++i) {
        target(block.active[static_cast<std::size_t>(i)],
               block.active[static_cast<std::size_t>(j)]) = C(i, j);
      }
    }
  }
  return out;
...
fit_expected<Eigen::VectorXd>
psd_lift_start(const PsdLiftLayout& layout,
               const EqConstraints& con,
  ...
  for (const auto& block : layout.blocks) {
    ...
    const Eigen::VectorXd values =
        eig.eigenvalues().array().max(eigen_floor);
    const Eigen::MatrixXd projected =
        eig.eigenvectors() * values.asDiagonal() *
        eig.eigenvectors().transpose();
    Eigen::LLT<Eigen::MatrixXd> llt(projected);
    ...
    for (Eigen::Index col = 0; col < q; ++col) {
      for (Eigen::Index row = col; row < q; ++row) {
        x(k++) = L(row, col);
      }
    }
  }
...
psd_lifted_evaluation(
  ...
  if (with_sigma_jacobian) {
    ...
    for (Eigen::Index k = 0;
      ...
      const auto mat = locations[static_cast<std::size_t>(k)].mat;
      if (mat == model::MatId::Theta || mat == model::MatId::Psi) {
        ...
        for (Eigen::Index lr = lc; lr < q; ++lr, ++local_col) {
          ...
          J.col(layout.n_alpha + block.lift_offset + local_col)
              .segment(sigma_offset[block.block],
                       detail::vech_len(dSigma.rows())) =
              detail::vech_lower(dSigma);
        }
      }
    }
...
struct PsdConstraintCallbacks {
  optim::ConstraintFn h;
  optim::ConstraintJacFn jacobian;
  Eigen::Index n_constraint = 0;
};

PsdConstraintCallbacks
psd_constraint_callbacks(const EqConstraints& con,
                         const NonlinearEqConstraints& nl,
                         const PsdLiftLayout& layout) {
  const Eigen::Index n_x = layout.n_alpha + layout.n_lift;
  const Eigen::Index n_constraint = layout.n_link + nl.m();

  PsdConstraintCallbacks out;
  out.n_constraint = n_constraint;
  out.h = [&con, &nl, &layout, n_constraint](
              const Eigen::VectorXd& x) {
    const Eigen::VectorXd alpha = x.head(layout.n_alpha);
    Eigen::VectorXd h(n_constraint);
    Eigen::Index out_row = 0;
    for (const auto& block : layout.blocks) {
      const Eigen::VectorXd c = block.c0 + block.D * alpha;
      const Eigen::Index q = block.active_dim();
    ...
    if (nl.active()) {
      h.tail(nl.m()) = nl.h(con.expand(alpha));
    }
    return h;
  };
  out.jacobian = [&con, &nl, &layout, n_constraint, n_x](
                     const Eigen::VectorXd& x) {
    const Eigen::VectorXd alpha = x.head(layout.n_alpha);
    Eigen::MatrixXd J = Eigen::MatrixXd::Zero(n_constraint, n_x);
    Eigen::Index out_row = 0;
    for (const auto& block : layout.blocks) {
      const Eigen::Index q = block.active_dim();
      const Eigen::MatrixXd L = lower_from_x(
    ...
    if (nl.active()) {
      J.block(out_row, 0, nl.m(), layout.n_alpha).noalias() =
          nl.jacobian(con.expand(alpha)) * con.K();
    }
    return J;
  };
  return out;
...
psd_ordinal_derivative_probe(
    ...
    double finite_difference_step,
    frontier::PsdFitOptions options) {
  constexpr const char* who = "psd_ordinal_derivative_probe";
  if (auto ok = validate_probe_step(finite_difference_step, who);
      !ok.has_value()) return std::unexpected(ok.error());
  if (auto ok = prepare_ordinal_partable(
...
drive_psd_problem(const optim::ScalarProblem& prob,
  ...
  attach_diagnostics(est, pt, pre, diagnostic_bounds);
  return est;
}
...
fit_ml_psd(spec::LatentStructure pt, const model::MatrixRep& rep,
  ...
  const optim::ScalarProblem prob =
      psd_ml_problem(pre->ev, rep, pre->con, *layout, samp,
                     std::move(*cache));
  const PsdConstraintCallbacks constraints =
      psd_constraint_callbacks(pre->con, pre->nl, *layout);
...
fit_ordinal_psd(spec::LatentStructure pt,
                ...
                OrdinalParameterization parameterization,
                PsdFitOptions psd_opts) {
  constexpr const char* who = "fit_ordinal_psd";
  ...
  auto ordinal_problem = psd_ordinal_problem(
      *pre, pt, rep, stats, *layout, *start, weights, parameterization);
  if (!ordinal_problem.has_value()) {
...
fit_gmm_constrained(spec::LatentStructure pt, const model::MatrixRep& rep,
                    ...
                    Bounds bounds, Backend backend, OptimOptions opts) {
  auto pre = prelude(pt, rep, samp, x0, "fit_gmm_constrained");
  if (!pre.has_value()) return std::unexpected(pre.error());
...
profile_fitted_weight_at(const spec::LatentStructure& pt,
  ...
  auto ev_or = model::ModelEvaluator::build(pt, rep);
  if (!ev_or.has_value()) {
    return std::unexpected(model_to_fit(ev_or.error(), who));
  }
  return build_fitted_weight(*ev_or, samp, theta, fitted_opts.kind, who);
}
...
fit_expected<double>
scalar_profile_scaling_factor(
    const spec::LatentStructure& pt,
    const robust::ParamSpaceSandwich& sw,
    const Estimates& constrained,
    const ScalarFunctional& functional,
    const char* who) {
  auto grad = scalar_gradient(functional, constrained.theta,
                              constrained.theta.size(), who);
  if (!grad.has_value()) return std::unexpected(grad.error());
  if (sw.A1.rows() != sw.A1.cols() || sw.B1.rows() != sw.B1.cols() ||
      sw.A1.rows() != sw.B1.rows()) {
    return std::unexpected(fit_err(FitError::Kind::NumericIssue,
        std::string(who) + ": robust profile sandwich matrices are not "
            "conformable"));
  }
  ...
  Eigen::VectorXd r;
  if (sw.K_con.size() > 0) {
    if (sw.K_con.rows() != grad->size() ||
        sw.K_con.cols() != sw.A1.rows()) {
      return std::unexpected(fit_err(FitError::Kind::NumericIssue,
          std::string(who) + ": robust profile sandwich K has incompatible "
              "shape"));
    }
    A = sw.A1;
    B = sw.B1;
    r = sw.K_con.transpose() * *grad;
  } else if (sw.A1.rows() == grad->size()) {
    ...
    if (K.rows() != grad->size()) {
      return std::unexpected(fit_err(FitError::Kind::NumericIssue,
          std::string(who) + ": constraint reparameterization has "
              "incompatible shape for robust profile scaling"));
    }
    A = (K.transpose() * sw.A1 * K).eval();
    B = (K.transpose() * sw.B1 * K).eval();
    r = K.transpose() * *grad;
  } else {
    return std::unexpected(fit_err(FitError::Kind::NumericIssue,
        std::string(who) + ": robust profile sandwich dimension does not "
            "match theta"));
  }
...
void fill_scaled_profile(ScalarProfileLrtResult& out, double scale) {
  out.scaling_factor = scale;
  out.T_scaled = out.T / scale;
  out.p_value_scaled = inference::chi2_pvalue(out.T_scaled, 1);
}

void fill_misspec_profile(ScalarProfileLrtResult& out, double scale) {
  out.misspec_scaling_factor = scale;
  out.T_misspec_scaled = out.T / scale;
  out.p_value_misspec_scaled =
      inference::chi2_pvalue(out.T_misspec_scaled, 1);
...
bool reference_needs_sandwich(ScalarProfileReference reference) noexcept {
...
bool reference_wants_robust_scaled(ScalarProfileReference reference) noexcept {
  return reference == ScalarProfileReference::RobustScaled;
}

bool reference_wants_misspec(ScalarProfileReference reference) noexcept {
  return reference == ScalarProfileReference::MisspecScaled ||
         reference == ScalarProfileReference::MisspecMixture;
}
...
fit_expected<robust::ParamSpaceSandwich>
profile_gmm_param_space_sandwich_observed(
    spec::LatentStructure pt, const model::MatrixRep& rep,
    const SampleStats& samp, const Estimates& est,
    const gmm::Weight& weight, const data::RawData& raw,
    GmmProfileRobustOptions robust_options) {
...
profile_lrt_scalar_ml(spec::LatentStructure pt, const model::MatrixRep& rep,
  ...
  auto constrained = fit_ml_constrained(
      spec::LatentStructure(pt), rep, samp, unrestricted.theta, std::move(extra),
  ...
  out.n_obs = *n_or;
  out.df = 1;
  if (robust_raw != nullptr && reference_wants_robust_scaled(reference)) {
    auto sw = robust::param_space_sandwich(
        spec::LatentStructure(pt), rep, samp, out.constrained, *robust_raw,
        robust::InferenceSpec{}, true);
    if (!sw.has_value()) return std::unexpected(post_to_fit(sw.error()));
  ...
  if (robust_raw != nullptr && reference_wants_misspec(reference)) {
    auto sw_misspec = robust::param_space_sandwich(
        spec::LatentStructure(pt), rep, samp, out.constrained, *robust_raw,
        robust::InferenceSpec{robust::Information::Observed,
                              robust::WeightMoments::Structured,
                              robust::ScoreCovariance::Empirical},
        true);
    if (!sw_misspec.has_value()) {
      return std::unexpected(post_to_fit(sw_misspec.error()));
    }
    auto misspec_scale = scalar_profile_scaling_factor(
        pt, *sw_misspec, out.constrained, functional, who);
    if (!misspec_scale.has_value()) {
      return std::unexpected(misspec_scale.error());
    }
    fill_misspec_profile(out, *misspec_scale);
  }
  return out;
}
...
profile_lrt_parameter_ml(spec::LatentStructure pt, const model::MatrixRep& rep,
  ...
  return profile_lrt_scalar_ml(std::move(pt), rep, samp, unrestricted,
                               ...
                               backend, opts, constraint_tol, robust_raw,
                               reference);
}
...
profile_lrt_scalar_gmm(spec::LatentStructure pt, const model::MatrixRep& rep,
  ...
  auto grad0 = scalar_gradient(functional, unrestricted.theta,
                               unrestricted.theta.size(), who);
  if (!grad0.has_value()) return std::unexpected(grad0.error());
  const gmm::Weight weight_for_scale = weight;
  ...
  auto constrained = fit_gmm_constrained(
      spec::LatentStructure(pt), rep, samp, unrestricted.theta, std::move(weight),
      std::move(extra), std::move(bounds), backend, opts);
  if (!constrained.has_value()) return std::unexpected(constrained.error());
  ...
  if (robust_raw != nullptr && reference_wants_misspec(reference)) {
    auto sw_misspec = profile_gmm_param_space_sandwich_observed(
        spec::LatentStructure(pt), rep, samp, out.constrained,
        weight_for_scale, *robust_raw, robust_options);
    if (!sw_misspec.has_value()) return std::unexpected(sw_misspec.error());
    auto misspec_scale = scalar_profile_scaling_factor(
        pt, *sw_misspec, out.constrained, functional, who);
    if (!misspec_scale.has_value()) {
      return std::unexpected(misspec_scale.error());
...
profile_lrt_scalar_gmm_fitted_weight(
  ...
  auto constrained = fit_gmm_fitted_weight_constrained(
      spec::LatentStructure(pt), rep, samp, unrestricted->theta,
      std::move(extra), fitted_opts, std::move(bounds), backend, opts);
  if (!constrained.has_value()) return std::unexpected(constrained.error());
  ...
  out.df = 1;
  if (robust_raw != nullptr && reference_wants_robust_scaled(reference)) {
    auto weight = profile_fitted_weight_at(
        pt, rep, samp, out.constrained.theta, fitted_opts, who);
    if (!weight.has_value()) return std::unexpected(weight.error());
    auto sw = continuous_ls_param_space_sandwich(
        spec::LatentStructure(pt), rep, samp, out.constrained, *weight,
        *robust_raw);
    if (!sw.has_value()) return std::unexpected(post_to_fit(sw.error()));
    auto scale = scalar_profile_scaling_factor(
        pt, *sw, out.constrained, functional, who);
  ...
  if (robust_raw != nullptr && reference_wants_misspec(reference)) {
    auto weight = profile_fitted_weight_at(
        pt, rep, samp, out.constrained.theta, fitted_opts, who);
    if (!weight.has_value()) return std::unexpected(weight.error());
    auto sw_misspec = continuous_ls_param_space_sandwich_observed(
        spec::LatentStructure(pt), rep, samp, out.constrained, *weight,
        *robust_raw);
    if (!sw_misspec.has_value()) {
      return std::unexpected(post_to_fit(sw_misspec.error()));
...
eval_profile_point(const ProfileEvaluator& eval, double target,
                   double cutoff, const ScalarProfileCiOptions& options,
                   const char* who) {
  auto r = eval(target);
  if (!r.has_value()) return std::unexpected(r.error());
  double stat = r->T;
  ...
  switch (options.reference) {
    case ScalarProfileReference::Ordinary:
      stat = r->T;
      break;
    case ScalarProfileReference::RobustScaled:
      stat = r->T_scaled;
      break;
    case ScalarProfileReference::MisspecScaled:
      stat = r->T_misspec_scaled;
      break;
    case ScalarProfileReference::MisspecMixture:
      stat = r->T;
      if (!std::isfinite(options.cutoff)) {
...
solve_profile_root_side(const ProfileEvaluator& eval,
  ...
  if (has_bound) {
    auto p = eval_profile_point(eval, outside_x, cutoff, options, who);
    ++evals;
  ...
  for (int k = 0; k < options.max_iter; ++k) {
    ...
    auto p = eval_profile_point(eval, mid, cutoff, options, who);
    ++evals;
    if (!p.has_value()) return std::unexpected(p.error());
...
profile_ci_from_evaluator(double estimate,
  ...
  out.cutoff = *cutoff;
  out.lower_cutoff =
      options.reference == ScalarProfileReference::MisspecMixture
...
profile_lrt_ci_parameter_ml(spec::LatentStructure pt,
  ...
  if (reference_needs_sandwich(ci_options.reference) && robust_raw == nullptr) {
    return std::unexpected(fit_err(FitError::Kind::NumericIssue,
        std::string(who) + ": robust/misspec CI requires raw data"));
  }
      ...
      [pt, &rep, &samp, unrestricted, parameter, bounds, backend, opts,
       ...
       reference = ci_options.reference](double target) mutable {
        ...
        return profile_lrt_parameter_ml(
            ...
            std::move(bounds_eval), backend, opts, constraint_tol, robust_raw,
            reference);
      };
      ...
      [pt, &rep, &samp, unrestricted, weight, parameter, bounds, backend, opts,
       ...
       reference = ci_options.reference](double target) mutable {
        ...
        return profile_lrt_parameter_gmm(
            ...
            constraint_tol, robust_raw, robust_options, reference);
      };
  return profile_ci_from_evaluator(estimate, ci_options, eval, who);
}
...
profile_lrt_ci_parameter_gmm_fitted_weight(
  ...
  ProfileEvaluator eval =
      [pt, &rep, &samp, unrestricted = *unrestricted, parameter, fitted_opts,
       bounds, backend, opts, constraint_tol, robust_raw,
       reference = ci_options.reference](double target) mutable {
        spec::LatentStructure pt_eval = pt;
        Bounds bounds_eval = bounds;
        return profile_lrt_parameter_gmm_fitted_weight(
            ...
            constraint_tol, robust_raw, reference);
      };
  return profile_ci_from_evaluator(estimate, ci_options, eval, who);
}
