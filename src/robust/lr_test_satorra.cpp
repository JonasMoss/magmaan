...
double symmetric_basis_trace(const Eigen::Ref<const Eigen::MatrixXd>& A,
  ...
  Eigen::Index e1_i[2] = {r1, c1};
  Eigen::Index e1_j[2] = {c1, r1};
  Eigen::Index e2_i[2] = {r2, c2};
  Eigen::Index e2_j[2] = {c2, r2};
  const int n1 = (r1 == c1) ? 1 : 2;
  const int n2 = (r2 == c2) ? 1 : 2;
  double out = 0.0;
  for (int a = 0; a < n1; ++a) {
    const Eigen::Index j = e1_i[a];
    const Eigen::Index k = e1_j[a];
    for (int b = 0; b < n2; ++b) {
      const Eigen::Index l = e2_i[b];
      const Eigen::Index i = e2_j[b];
      out += A(i, j) * A(k, l);
    }
  }
  return out;
...
fiml_lavaan_expected_wls_v(
  ...
  if (has_mask && raw.mask.size() != B) {
    return std::unexpected(make_err(PostError::Kind::NumericIssue,
        "fiml_lavaan_expected_wls_v: malformed missingness mask"));
  }
  ...
  for (std::size_t b = 0; b < B; ++b) {
    ...
    if (has_mask && (raw.mask[b].rows() != raw.X[b].rows() ||
                     raw.mask[b].cols() != p)) {
      return std::unexpected(make_err(PostError::Kind::NumericIssue,
          "fiml_lavaan_expected_wls_v: mask block shape mismatch"));
    }
  }

  Eigen::MatrixXd W = Eigen::MatrixXd::Zero(total, total);
  Eigen::Index off = 0;
  ...
  for (std::size_t b = 0; b < B; ++b) {
    ...
    if (n <= 0) {
      return std::unexpected(make_err(PostError::Kind::NumericIssue,
          "fiml_lavaan_expected_wls_v: empty raw block"));
    }

    Eigen::MatrixXd Wb = Eigen::MatrixXd::Zero(q_b[b], q_b[b]);
    for (Eigen::Index row = 0; row < n; ++row) {
      obs.clear();
      obs.reserve(static_cast<std::size_t>(p));
      for (Eigen::Index j = 0; j < p; ++j) {
        const double x = X(row, j);
        const bool observed = has_mask ? (raw.mask[b](row, j) != 0)
                                       : std::isfinite(x);
        if (observed && !std::isfinite(x)) {
          return std::unexpected(make_err(PostError::Kind::NumericIssue,
              "fiml_lavaan_expected_wls_v: mask marks a non-finite value "
              "as observed"));
        }
      ...
      if (obs.empty()) {
        return std::unexpected(make_err(PostError::Kind::NumericIssue,
            "fiml_lavaan_expected_wls_v: empty observed row"));
      }
      ...
      Wb.topLeftCorner(p, p).noalias() += A_full;
      for (Eigen::Index c1 = 0; c1 < p; ++c1) {
        for (Eigen::Index r1 = c1; r1 < p; ++r1) {
          const Eigen::Index k1 = detail::vech_index(p, r1, c1);
          for (Eigen::Index c2 = 0; c2 < p; ++c2) {
            for (Eigen::Index r2 = c2; r2 < p; ++r2) {
              const Eigen::Index k2 = detail::vech_index(p, r2, c2);
              Wb(p + k1, p + k2) +=
                  0.5 * symmetric_basis_trace(A_full, r1, c1, r2, c2);
            }
          }
        }
      }
    ...
    Wb /= static_cast<double>(n);
    W.block(off, off, q_b[b], q_b[b]) = 0.5 * (Wb + Wb.transpose()).eval();
    off += q_b[b];
  }
  return W;
}
...
fiml_lavaan_model_gamma_block(
  ...
  if (block >= raw.X.size()) {
    return std::unexpected(make_err(PostError::Kind::NumericIssue,
        "fiml_lavaan_model_gamma_block: block index out of range"));
  }
  ...
  if (n <= 0 || p <= 0 || mu.size() != p ||
    ...
    return std::unexpected(make_err(PostError::Kind::NumericIssue,
        "fiml_lavaan_model_gamma_block: malformed block moments"));
  }
  const bool has_mask = !raw.mask.empty();
  if (has_mask) {
    if (raw.mask.size() != raw.X.size() ||
      ...
      return std::unexpected(make_err(PostError::Kind::NumericIssue,
          "fiml_lavaan_model_gamma_block: malformed missingness mask"));
    }
  }
  ...
  Eigen::VectorXd z(q);
  std::vector<std::uint8_t> ok(static_cast<std::size_t>(q), 0);
  std::vector<std::uint8_t> obs(static_cast<std::size_t>(p), 0);

  for (Eigen::Index r = 0; r < n; ++r) {
    std::fill(ok.begin(), ok.end(), static_cast<std::uint8_t>(0));
    ...
    for (Eigen::Index j = 0; j < p; ++j) {
      ...
      if (observed && !std::isfinite(x)) {
        return std::unexpected(make_err(PostError::Kind::NumericIssue,
            "fiml_lavaan_model_gamma_block: mask marks a non-finite value "
            "as observed"));
      }
      if (observed) {
        obs[static_cast<std::size_t>(j)] = 1;
        ok[static_cast<std::size_t>(j)] = 1;
        z(j) = x - mu(j);
      }
    }
    ...
    for (Eigen::Index c = 0; c < p; ++c) {
      for (Eigen::Index i = c; i < p; ++i) {
        ...
        if (observed) {
          z(k) = (X(r, i) - mu(i)) * (X(r, c) - mu(c)) - Sigma(i, c);
          ok[static_cast<std::size_t>(k)] = 1;
        }
        ++k;
      }
    }
    ...
    for (Eigen::Index a = 0; a < q; ++a) {
      if (!ok[static_cast<std::size_t>(a)]) continue;
      for (Eigen::Index b = 0; b <= a; ++b) {
        if (ok[static_cast<std::size_t>(b)]) {
          Gamma(a, b) += z(a) * z(b);
        }
      }
    }
...
compute_fiml_satorra2000_lavaan_convention(
  ...
  const std::size_t B = raw.X.size();
  if (B == 0 || W_full.rows() != Delta1_alpha.rows() ||
      W_full.cols() != Delta1_alpha.rows() ||
      implied.sigma.size() != B || observed_info.rows() != basis_H1.rows() ||
      observed_info.cols() != basis_H1.rows()) {
    return std::unexpected(make_err(PostError::Kind::NumericIssue,
        "compute_fiml_satorra2000_lavaan_convention: malformed inputs"));
  }
  ...
  for (std::size_t b = 0; b < B; ++b) {
    if (b >= sm.n_obs.size() || sm.n_obs[b] <= 0 ||
      ...
      return std::unexpected(make_err(PostError::Kind::NumericIssue,
          "compute_fiml_satorra2000_lavaan_convention: malformed group n"));
    }
    N += static_cast<double>(sm.n_obs[b]);
  }
  if (!(N > 0.0)) {
    return std::unexpected(make_err(PostError::Kind::NumericIssue,
        "compute_fiml_satorra2000_lavaan_convention: total N must be positive"));
  }

  Eigen::MatrixXd A1 =
      basis_H1.transpose() * (observed_info / N) * basis_H1;
  A1 = 0.5 * (A1 + A1.transpose()).eval();

  Eigen::MatrixXd B1 =
      Eigen::MatrixXd::Zero(Delta1_alpha.cols(), Delta1_alpha.cols());
  Eigen::Index off = 0;
  for (std::size_t b = 0; b < B; ++b) {
    const Eigen::Index p = raw.X[b].cols();
    const Eigen::Index q = p + detail::vech_len(p);
    if (off + q > W_full.rows() || Delta1_alpha.rows() < off + q ||
  ...
  if (off != W_full.rows()) {
    return std::unexpected(make_err(PostError::Kind::NumericIssue,
        "compute_fiml_satorra2000_lavaan_convention: block offsets do not "
        "span saturated information"));
  }
...
post_expected<SatorraDiffResult>
compute_ml2s_satorra2000_lavaan_convention(
    const data::RawData& raw,
    const estimate::fiml::SaturatedMoments& sm,
    const model::ImpliedMoments& implied,
    const Eigen::Ref<const Eigen::MatrixXd>& Delta1_alpha,
    ...
    const Eigen::Ref<const Eigen::MatrixXd>& A_alpha) {
  const std::size_t B = raw.X.size();
  if (B == 0 || V.rows() != Delta1_alpha.rows() ||
      V.cols() != Delta1_alpha.rows() ||
      implied.sigma.size() != B) {
    return std::unexpected(make_err(PostError::Kind::NumericIssue,
        "compute_ml2s_satorra2000_lavaan_convention: malformed inputs"));
  }
  ...
  for (std::size_t b = 0; b < B; ++b) {
    if (b >= sm.n_obs.size() || sm.n_obs[b] <= 0 ||
        raw.X[b].rows() != sm.n_obs[b]) {
      return std::unexpected(make_err(PostError::Kind::NumericIssue,
          "compute_ml2s_satorra2000_lavaan_convention: malformed group n"));
    }
    N += static_cast<double>(sm.n_obs[b]);
  }
  if (!(N > 0.0)) {
    return std::unexpected(make_err(PostError::Kind::NumericIssue,
        "compute_ml2s_satorra2000_lavaan_convention: total N must be positive"));
  }
  ...
  for (std::size_t b = 0; b < B; ++b) {
    const Eigen::Index p = raw.X[b].cols();
    const Eigen::Index q = p + detail::vech_len(p);
    if (off + q > V.rows() || Delta1_alpha.rows() < off + q ||
        implied.sigma[b].rows() != p || implied.sigma[b].cols() != p) {
      return std::unexpected(make_err(PostError::Kind::NumericIssue,
          "compute_ml2s_satorra2000_lavaan_convention: block shape mismatch"));
    }
    Eigen::VectorXd mu = Eigen::VectorXd::Zero(p);
    if (b < implied.mu.size() && implied.mu[b].size() == p) {
      mu = implied.mu[b];
      ...
      return std::unexpected(make_err(PostError::Kind::NumericIssue,
          "compute_ml2s_satorra2000_lavaan_convention: implied mean shape "
          "mismatch"));
    }
    auto Gamma_or = fiml_lavaan_model_gamma_block(
        raw, b, mu, implied.sigma[b]);
    ...
    const double f_b = static_cast<double>(sm.n_obs[b]) / N;
    const Eigen::MatrixXd Wb = V.block(off, off, q, q);
    const Eigen::MatrixXd Db = Delta1_alpha.block(off, 0, q, Delta1_alpha.cols());
    const Eigen::MatrixXd WDb = Wb * Db;
    A1.noalias() += f_b * Db.transpose() * Wb * Db;
    B1.noalias() += f_b * WDb.transpose() * (*Gamma_or) * WDb;
    off += q;
  }
  if (off != V.rows()) {
    return std::unexpected(make_err(PostError::Kind::NumericIssue,
        "compute_ml2s_satorra2000_lavaan_convention: block offsets do not "
        "span weight matrix"));
  }
  A1 = 0.5 * (A1 + A1.transpose()).eval();
  B1 = 0.5 * (B1 + B1.transpose()).eval();
...
lr_test_satorra2000_fiml_from_data_impl(
  ...
  if (gamma == GammaSource::NT) {
    ...
    if (convention == SatorraMomentConvention::Lavaan) {
      auto ev_or = model::ModelEvaluator::build(pt_H1, rep_H1);
      if (!ev_or.has_value()) {
        return std::unexpected(make_err(PostError::Kind::NumericIssue,
            "lr_test_satorra2000_fiml_from_data: ModelEvaluator::build "
            "failed for lavaan convention: " + ev_or.error().detail));
      }
      auto eval_or = ev_or->evaluate(theta_H1_full, false, false);
...
lr_test_satorra2000_ml2s_from_data(
  ...
  post_expected<SatorraDiffResult> sd_or =
      std::unexpected(make_err(PostError::Kind::NumericIssue,
          "lr_test_satorra2000_ml2s_from_data: internal initialization error"));
  if (gamma == GammaSource::NT ||
      convention == SatorraMomentConvention::Magmaan) {
    sd_or = compute_fiml_satorra2000(Delta1_alpha, V, Gamma, A_alpha);
  } else {
    auto ev_or = model::ModelEvaluator::build(pt_H1, rep_H1);
    if (!ev_or.has_value()) {
      return std::unexpected(make_err(PostError::Kind::NumericIssue,
          "lr_test_satorra2000_ml2s_from_data: ModelEvaluator::build failed "
          "for lavaan convention: " + ev_or.error().detail));
    }
    auto eval_or = ev_or->evaluate(theta_H1_full, false, false);
    if (!eval_or.has_value()) {
      return std::unexpected(make_err(PostError::Kind::NumericIssue,
          "lr_test_satorra2000_ml2s_from_data: ModelEvaluator::evaluate "
          "failed for lavaan convention: " + eval_or.error().detail));
    }
    sd_or = compute_ml2s_satorra2000_lavaan_convention(
        raw, sm, eval_or->moments, Delta1_alpha, V, A_alpha);
  }
  if (!sd_or.has_value()) return std::unexpected(sd_or.error());
  auto out_or = lr_test_satorra2000(T_H0 - T_H1, *sd_or);
  if (!out_or.has_value()) return std::unexpected(out_or.error());
