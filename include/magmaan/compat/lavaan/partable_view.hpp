...
// which magmaan doesn't model yet. `plabel` is `.p<id>.` for formula rows, "" for
...
LavaanParTable to_lavaan_partable(const spec::LatentStructure& structure,
...
// `spec::LatentNames` companion from a lavaan-shaped partable. Used by the R
