...
## `group.equal` keyword: the configural -> metric Satorra-2000 scaled LRT and
## the FMG eigenvalue-tail diagnostics on the difference spectrum, cross-checked
## against lavaan.
##
## Run from the repo root after installing the package:
##     Rscript r-package/examples/group_equal_nested_ordinal.R
##
## H1 = configural (no group.equal), H0 = metric (group.equal thresholds +
## loadings, theta). The Wu-Estabrook release makes the pair non-nested (metric
## frees the group-2 scale/intercepts configural pins), so -- like lavaan -- the
## nested test uses A.method = "delta". nestedTest() returns the difference
## spectrum, and fmg_nested_ordinal() applies the FMG transforms to it the same
## way fmg_tests_ordinal() does for a single-model spectrum. The oracle is
## lavaan's `lavTestLRT(method = "satorra.2000")` on robust (`se = "robust.sem"`)
## DWLS-theta fits; the C++ golden anchors the same 3.025 / df 3 / 0.388.
##
...
ge       <- c("thresholds", "loadings")

## ---- configural + metric ordinal DWLS-theta fits (one shared stats object) --
...
## ---- nested Satorra-2000 (delta) vs lavaan lavTestLRT ----------------------
res <- magmaan::nestedTest(fit_cfg, fit_met, data = stats,
                           method = "satorra.2000", A.method = "delta",
                           weight = "DWLS")
N <- fit_cfg$ntotal; G <- fit_cfg$ngroups
mg_scaled <- res$T_scaled * (N - G) / N   # 2N*fmin -> lavaan (N-G)/N convention

lav_cfg <- lavaan::cfa(model, data = df, ordered = ordered, estimator = "DWLS",
                       parameterization = "theta", group = "school",
                       se = "robust.sem", test = "satorra.bentler")
lav_met <- lavaan::cfa(model, data = df, ordered = ordered, estimator = "DWLS",
                       parameterization = "theta", group = "school",
                       group.equal = ge, se = "robust.sem",
                       test = "satorra.bentler")
lr <- lavaan::lavTestLRT(lav_cfg, lav_met, method = "satorra.2000")
lav_scaled <- as.numeric(lr[["Chisq diff"]][2])
lav_df     <- as.integer(lr[["Df diff"]][2])
lav_p      <- as.numeric(lr[["Pr(>Chisq)"]][2])

cat("== configural -> metric Satorra-2000 (delta, theta) ==\n")
...
stopifnot(abs(mg_scaled - lav_scaled) < 5e-3)
stopifnot(abs(res$p_scaled - lav_p) < 5e-3)

## ---- FMG eigenvalue-tail diagnostics on the difference spectrum ------------
fr <- magmaan::fmg_nested_ordinal(fit_cfg, fit_met, stats, A.method = "delta")
cat("\n== fmg_nested_ordinal (default tests) ==\n")
print(fr[, c("label", "base_statistic", "df", "p_value", "chi2_equiv")])

# Same shape as fmg_tests_ordinal: 5 default rows, all the difference rank, with
# the unscaled LS difference as the common base statistic.
...
# The SB row IS the Satorra-Bentler mean scaling of the difference triple, so it
# reproduces nestedTest()'s satorra.2000 scaled statistic.
