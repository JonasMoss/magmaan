...
##
##   inference_information_expected(fit)            expected (Fisher)
##                                                  information: E[-Hessian],
##                                                  a function of the model and
##                                                  the implied moments.
##   inference_information_observed_fd(fit)         observed information: the
##                                                  actual Hessian at theta-hat,
##                                                  by central differences.
##   inference_information_observed_analytic(fit)   observed information in
##                                                  closed form.
##
...
## matrix changes.
se_from <- function(info) core$inference_se(
  core$inference_vcov_partable(info, fit$partable))

se_exp <- se_from(core$inference_information_expected(fit))
se_ofd <- se_from(core$inference_information_observed_fd(fit))
se_oan <- se_from(core$inference_information_observed_analytic(fit))
...
lav_exp <- lavaan::cfa(model, data = df, information = "expected")
lav_obs <- lavaan::cfa(model, data = df, information = "observed")
