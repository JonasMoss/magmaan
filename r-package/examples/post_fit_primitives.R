...
vc_fit <- core$infer_vcov_fit(info, fit)
vc_parts <- core$infer_vcov_partable(info, fit$partable)
stopifnot(close(vc_parts, vc_fit))

se <- core$infer_se(vc_parts)
z_fit <- core$infer_z_test_fit(fit, se)
z_theta <- core$infer_z_test_theta(fit$theta, se)
stopifnot(same_list(z_theta, z_fit, c("z", "pvalue")))

R <- matrix(0, nrow = 1L, ncol = length(fit$theta))
R[1L, 1L] <- 1
wald_fit <- core$infer_wald_test_fit(fit, R, vc_parts)
wald_theta <- core$infer_wald_test_theta(fit$theta, R, vc_parts)
stopifnot(same_list(wald_theta, wald_fit, c("chi2", "df", "pvalue")))

rls_fit <- core$infer_rls_chi2_fit(fit, implied)
rls_sample <- core$infer_rls_chi2_sample(ss, implied)
...
uf_fit <- core$infer_build_u_factor_fit(fit)
uf_parts <- core$infer_build_u_factor_parts(fit$partable, ss, fit$theta)
...
rse_raw_fit <- core$infer_robust_se_raw_fit(fit, X)
rse_raw_parts <- core$infer_robust_se_raw_parts(fit$partable, ss, fit$theta, X)
...
rse_gamma <- core$infer_robust_se_parts(fit$partable, ss, fit$theta, gamma_hat)
