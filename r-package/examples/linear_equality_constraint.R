...
## Both are enforced by `magmaan_core$fit_fit()` via an affine reparam θ = θ₀ + Kα — the
...
S  <- magmaan_core$data_sample_stats_from_raw(list(as.matrix(hs[paste0("x", 1:3)])))

## ---- 1) a linear `==` constraint: λ_x2 + λ_x3 == 1.5 ----------------------
m  <- "visual =~ x1 + b2*x2 + b3*x3\nb2 + b3 == 1.5"
pt <- magmaan_core$lavaan_lavaanify(m)                     # the `==` row rides as op == "=="
fit <- magmaan_core$fit_fit(pt, S)                      # from_lavaan_partable re-parses it
...
se   <- magmaan_core$infer_se(magmaan_core$infer_vcov_partable(magmaan_core$infer_information_expected(fit), fit$partable))
lav <- cfa(m, hs)
...
m2  <- "visual =~ x1 + x2 + x3"
pt2 <- magmaan_core$lavaan_lavaanify(m2, effect_coding = TRUE)   # synthesizes `.p1.+.p2.+.p3. == 3`
fit2 <- magmaan_core$fit_fit(pt2, S)
T2   <- magmaan_core$infer_chi2_stat(magmaan_core$fit_sample_stats(fit2), fit2$fmin);  df2 <- magmaan_core$infer_df_stat(fit2$partable, magmaan_core$fit_sample_stats(fit2))
...
fit_mk <- magmaan_core$fit_fit(magmaan_core$lavaan_lavaanify(m_mk), S)
T_mk   <- magmaan_core$infer_chi2_stat(magmaan_core$fit_sample_stats(fit_mk), fit_mk$fmin);  df_mk <- magmaan_core$infer_df_stat(fit_mk$partable, magmaan_core$fit_sample_stats(fit_mk))
