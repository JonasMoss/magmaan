...
#
# simulation script model 'M3' study 2 
...
# three omitted cross-loadings
model3 <- '
# factor loadings
eta1 =~ y1 + start(1)*y2 + start(1)*y3 + 0*y4
...
FIT <- sem(model3)              # model zonder bounds 
...
for(ss in 1:length(SAMPLE.SIZES)) {
  ...
  for(i in seq_len(REP)) {
    ...
    # generate data
    Data <- simulateData(pop.model3, sample.nobs = N, empirical = FALSE)


    # ML
    fit <- try(sem(model3, data = Data, estimator = "ML",
...
save(ML,    file = "M3ML.RData")
save(OVVAR, file = "M3OVVAR.RData")
save(STAND, file = "M3STAND.RData")
save(WIDE,  file = "M3WIDE.RData")
