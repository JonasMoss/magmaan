...
#
# simulation script model 'M1' study 2 
...
FIT <- sem(model1)              # model zonder bounds 
...
for(ss in 1:length(SAMPLE.SIZES)) {
  ...
  for(i in seq_len(REP)) {
    ...
    # generate data
    Data <- simulateData(pop.model1, sample.nobs = N, empirical = FALSE)


    # ML
    fit <- try(sem(model1, data = Data, estimator = "ML",
...
save(ML,    file = "M1ML.RData")
save(OVVAR, file = "M1OVVAR.RData")
save(STAND, file = "M1STAND.RData")
save(WIDE,  file = "M1WIDE.RData")
