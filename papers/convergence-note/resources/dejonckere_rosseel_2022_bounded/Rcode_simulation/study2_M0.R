...
#
# simulation script model 'M0' study 2 
...
eta2 =~ y4 + start(1)*y5 + start(1)*y6 + start(0.3)*y7
eta3 =~ y7 + start(1)*y8 + start(1)*y9 + start(0.3)*y6
...
FIT <- sem(model0)              # model zonder bounds 
...
for(ss in 1:length(SAMPLE.SIZES)) {
  ...
  for(i in seq_len(REP)) {
    ...
    # generate data
    Data <- simulateData(pop.model0, sample.nobs = N, empirical = FALSE)


    # ML
    fit <- try(sem(model0, data = Data, estimator = "ML",
...
save(ML,    file = "M0ML.RData")
save(OVVAR, file = "M0OVVAR.RData")
save(STAND, file = "M0STAND.RData")
save(WIDE,  file = "M0WIDE.RData")
