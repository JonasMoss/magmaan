data {
  ...
  real<lower = -1, upper =1> pGC[3]; // prior beta correlation
}
transformed data{
	real epsilon = 0.01;
...
parameters {
	...
	vector<lower = epsilon, upper=p1epsilon>[3] lambda1; // first factor loadings
	vector<lower = n1epsilon, upper=p1epsilon>[J-3] lambda2; // remaining factor loadings
...
transformed parameters{
  ...
  lambda[5] = lambda1[2];
  lambda[9] = lambda1[3];
  lambda[2:4] = lambda2[1:3];
  lambda[6:8] = lambda2[4:6];
  lambda[10:11] = lambda2[7:8];
  ...
  for(j in 2:J){
    for (k in 1:(j-1)) {
     Sigma[j,k] = sdX[j]*lambda[j]*sdX[k]*lambda[k]*correl[dd[j],dd[k]];
     Sigma[k,j] = Sigma[j,k];
    }
  }
}
