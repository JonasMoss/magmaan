...
##        required if other pop model is considered.
## ---------------------------
...
OUT_MB_18ova <- vector("list", length=N)
...
for(r in 1:N) {
  ...
  OUT_MB_18ova[[r]] <- data.frame(NAME = NAME, SAMPLE.SIZE = SAMPLE.SIZES, 
...
names(OUT_MB_18ova) <- c("LW", "MBLW", "ML")


## ----------------------------------
##  Table: nonconvergence  
## ----------------------------------
Table18ov_convergence <- cbind(SAMPLE.SIZES,
                               OUT_MB_18ova$MBLW$BAD, 
                               OUT_MB_18ova$LW$BAD,
                               OUT_MB_18ova$ML$BAD)
colnames(Table18ov_convergence) <- c("sample size", "MBLW", "L&W", "ML")
Table18ov_convergence
...
##  Table: mean bias
## ----------------------------------
Table18ov_bias <- cbind(SAMPLE.SIZES, 
                        (OUT_MB_18ova$MBLW$MEAN - 0.25)*100,
                        (OUT_MB_18ova$LW$MEAN   - 0.25)*100,
                        (OUT_MB_18ova$ML$MEAN   - 0.25)*100)
colnames(Table18ov_bias) <- c("sample size", "MBLW", "L&W", "ML")
round(Table18ov_bias)
...
##  Table: MSE
## ----------------------------------
Table18ov_mse <- cbind(SAMPLE.SIZES,
                       OUT_MB_18ova$MBLW$MSE*100,
                       OUT_MB_18ova$LW$MSE*100,
                       OUT_MB_18ova$ML$MSE*100)
colnames(Table18ov_mse) <- c("sample size", "MBLW", "L&W", "ML")
round(Table18ov_mse)
