...
OUT_MB_14ova <- vector("list", length=N)
...
for(r in 1:N) {
  ...
  OUT_MB_14ova[[r]] <- data.frame(NAME = NAME, SAMPLE.SIZE = SAMPLE.SIZES, 
...
names(OUT_MB_14ova) <- c("LW", "MBLW", "ML")
...
## ----------------------------------
Table14ov_convergence <- cbind(SAMPLE.SIZES,
                               OUT_MB_14ova$MBLW$BAD, 
                               OUT_MB_14ova$LW$BAD,
                               OUT_MB_14ova$ML$BAD)
colnames(Table14ov_convergence) <- c("sample size", "MBLW", "L&W", "ML")
Table14ov_convergence
...
##  Table: mean bias
## ----------------------------------
Table14ov_bias <- cbind(SAMPLE.SIZES, 
                       (OUT_MB_14ova$MBLW$MEAN - 0.25)*100,
                       (OUT_MB_14ova$LW$MEAN   - 0.25)*100,
                       (OUT_MB_14ova$ML$MEAN   - 0.25)*100)
colnames(Table14ov_bias) <- c("sample size", "MBLW", "L&W", "ML")
round(Table14ov_bias)
...
##  Table: MSE
## ----------------------------------
Table14ov_mse <- cbind(SAMPLE.SIZES,
                       OUT_MB_14ova$MBLW$MSE*100,
                       OUT_MB_14ova$LW$MSE*100,
                       OUT_MB_14ova$ML$MSE*100)
colnames(Table14ov_mse) <- c("sample size", "MBLW", "L&W", "ML")
round(Table14ov_mse)
