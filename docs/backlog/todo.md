...
- **Done.** **Kline-corpus parity coverage for the Guo measurement-invariance
  ...
  `lavaanify_test.cpp`. End-to-end parity now lands two ways in
  `tests/golden/textbook_corpus_golden_test.cpp`: a compact checked-in export
  (`tests/fixtures/textbook_corpus/case_exports.json`, regenerated by
  `tests/tools/regen_textbook_case_fixtures.R`) gates the four Guo invariance
  rungs without requiring the submodule at test time, and a submodule-backed
  cross-check reads the finalized per-case corpus at
  `corpus/textbook-corpus/cases/kline_2023/` when available. Both paths re-fit
  from per-group summary statistics and assert order-free parity: df exact,
  magmaan's fmin no worse than lavaan, and tight two-sided chisq (<1e-3) for the
  well-converged rungs (configural df=8, weak df=11 / chisq 25.5159,
  ...
  reason documented in the tests. **Follow-up (deferred):** per-parameter θ̂/SE parity — the submodule
