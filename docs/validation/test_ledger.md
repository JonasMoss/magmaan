...
| Complete-data ML and LS estimation | lavaan JSON fixtures, real-data parity fixtures, and covariance-domain invariants | Unit, golden, and parity tests under `estimate` and `parity` | `tests/unit/ml_test.cpp`, `tests/unit/ls_path_test.cpp`, `tests/unit/psd_ml_test.cpp`, `tests/golden/ls_golden_test.cpp`, `tests/golden/lavaan_parity_golden_test.cpp`, `r-package/tests/testthat/test-admissibility.R` | Covariance-honest fixed-weight and expected-information fitted-weight GMM have derivative, interior-equivalence, weight-update/final-objective, repair, and R-contract gates; broader estimator-specific boundary/corpus/timing validation remains deliberately open. |
