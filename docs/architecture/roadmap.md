...
- Initial NORTA marginals are standard normal, standardized lognormal, Tukey
  ...
  a separate numerical integration policy. Johnson marginals use monotone
  transforms of a standard normal variate, fit SU/SB shape parameters to target
  skewness and excess kurtosis by deterministic quadrature, and plug into the
  same NORTA/independent-generator transform surface.
...
- Moment-matching now has an explicit simulation API:
  ...
  quantiles to PearsonDS 1.3.2. `MomentMatchFamily::Johnson` fits Johnson SU
  and SB marginals numerically against the same two shape moments. Checked
  fixtures generated by `tests/tools/regen_johnson_sim_fixtures.R` compare the
  fitted shape pair (gamma, delta), the SU/SB type, and quantiles to SuppDists
  1.1.9.9 `JohnsonFit`/`qJohnson`; because SuppDists's own moment fit is only
