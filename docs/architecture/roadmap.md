...
- The first paper-corpus scout layer is checked in under
  `tests/fixtures/paper_corpus/scout_manifest.json`, regenerated by
  `tests/tools/scout_paper_corpus.R`. It inventories seven public OSF seed
  nodes, scans only small code files for lavaan-shaped signals, records file
  sizes and OSF hashes/URLs, and keeps all raw downloads transient. The compact
  `zxqvn` empirical mediation/SEM project is the first promoted paper-corpus
  case: `tests/tools/regen_paper_corpus_fixtures.R` reads ignored raw files
  from `external/paper_corpus/zxqvn/raw/` and writes
  `tests/fixtures/paper_corpus/zxqvn_reference.json`, a core complete-data ML
  point-estimate fixture. The source script's clustered-SE option is catalogued
  but not yet part of the core parity surface. The next promotion candidate is
  the richer `hwkem` measurement-invariance tutorial.
