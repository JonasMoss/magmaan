...
- The first paper-corpus scout layer is checked in under
  ...
  sizes and OSF hashes/URLs, and keeps all raw downloads transient. The first
  promotion candidates are the `hwkem` measurement-invariance tutorial and the
  compact `zxqvn` empirical mediation/SEM project; both still need explicit
  license/data-handling notes and lavaan oracle promotion before becoming
  parity fixtures.
