...
- Frontier profile-LR scalar infrastructure for complete-data ML and
  ...
  `frontier_profile_lrt_ci_parameter_*`. Robust scaling, Bartlett /
  small-sample correction, functional R callbacks, and ordinal fitted-
...
- Validation: `tests/golden/twolevel_golden_test.cpp` drives the public api end
  to end against checked-in lavaan oracle fixtures
  (`tests/fixtures/twolevel/*.json`, regenerated by
  `tests/tools/regen_oracle_twolevel.R`), asserting theta-hat, standard SEs,
  chi-square, and df. Unbalanced full-information cells are additionally
  cross-checked against the Mplus 9.1 Demo via
  `tests/tools/regen_oracle_twolevel_mplus.R` (a maintainer tool; CI invokes
  neither R nor Mplus).
- Remaining work (see [docs/backlog/todo.md](../backlog/todo.md)): multi-group
