...
| `data::ordinal_stats_from_integer_data` | `data_ordinal_stats_from_raw` / `data_ordinal_stats_from_raw_impl` | good | Prefer canonical name; keep `_impl` as compatibility alias. | Helper `data_ordinal_stats_from_df` remains plain R composition. |
...
| `robust::*` U-Gamma / weighted-chi-square helpers | `robust_*` / old `infer_*` robust names | good | Prefer canonical names; keep old spellings as compatibility aliases. | The functions are robust-test primitives, not generic inference. |
