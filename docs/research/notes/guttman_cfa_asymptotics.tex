...
\[
  ...
  \{2\binom{p-1}{2}\}^{-1}
  \left\{
    \sum_{j=1}^{p}\sum_{\substack{\ell=1\\ \ell\ne j}}^{p}
      \frac{S_{ij}S_{i\ell}}{S_{j\ell}}
    -2(p-1)S_{ii}
  \right\}.
  \end{aligned}
...
to \(S_{ii}\). The ordered double sum counts each unordered pair twice, so it
subtracts \(2(p-1)S_{ii}\) and divides by \(2\binom{p-1}{2}\). In a
multi-factor simple-structure block, replace \(p\) by the block size \(p_f\)
and sum only within the block. Under an exact congeneric one-factor population,
this population rule recovers \(h_i=\lambda_i^2\phi\).
