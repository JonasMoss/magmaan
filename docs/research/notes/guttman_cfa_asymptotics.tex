...
\[
  ...
  P_i=\{(j,\ell):j<\ell,\ j\ne i,\ \ell\ne i\},
\]
with \(R_{ab}=S_{ab}/(S_{aa}S_{bb})^{1/2}\). The second equality is just the
cancellation of the item variances in the correlation expression. Here
\(|P_i|=\binom{p-1}{2}\), so the denominator is constant across \(i\). In a
multi-factor simple-structure block, replace \(p\) by the block size \(p_f\);
then \(|P_i|\) is constant within a block, but not necessarily across blocks.
Under an exact congeneric one-factor population, this population rule recovers
\(h_i=\lambda_i^2\phi\).
