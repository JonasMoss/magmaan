...
\section{Computational Jacobian Accounting}
...
magnitude slower than NT/ULS: the expensive part is how \(J\) is assembled.
...
Restricted Guttman remains different in this pass. Active residual and loading
constraints change the estimator map through a constrained communality solve
and a loading projection. The current restricted SE path therefore still uses
central finite differences, requiring \(2p^\star\) full restricted-map
evaluations per covariance block. Making that path competitive requires
differentiating the active KKT/projection system, including any active
residual-communality constraints and loading equality rows.
