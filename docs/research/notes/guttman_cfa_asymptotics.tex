...
refits were being used. The default \texttt{gmm\_block} restricted path now
batches the constrained \(h^2\) derivative: it builds the block-active GMM RHS
...
numbers are about \(2.7\), \(3.7\), and \(3.9\) ms. Non-default restricted
communality rules remain analytic but may fall back to the direction-wise
assembly until their constrained RHS columns are batched too.

A later restricted-cost pass removed a smaller remaining duplication. The
single-block fast path now constructs the base restricted block from the
batched derivative result instead of first rebuilding the same block for
validation, and the loading projection applies
\[
