...
\item The nested, degenerate case is \emph{not} a clean corollary. The limiting
  ...
  covariance quadratic of the fitted residual fluctuation. A meat-only
  substitution $\Gamma\mapsto(W-\Wr)\Gamma(W-\Wr)\T$ into the $U$-matrix does
  not produce that Hessian (Proposition~\ref{prop:nested}). It needs its own
  derivation.
\end{enumerate}
...
For one model $M$ in the common case $W=W(u)$, let
\[
...
\begin{proposition}
\label{prop:nested}
Let $x$ be the first-stage vector with $\sqrt N(\hat x-x_{0})\to Z\sim
N(0,\Gamma_{x})$, let $u=P x$, and let $W=W(x)$. Write
$f_{M}(x)=\min_{\theta}\tfrac12 r_{M}(\theta,Px)\T W(x)r_{M}(\theta,Px)$ and
$s_{M}(x)=\nabla_{x}f_{M}(x)$.
...
profile gradients agree at $x_{0}$.
For directions $a,b$ in $x$-space write $h=Pa$, $k=Pb$,
$W_{a}=DW(x_{0})[a]$, and $W_{ab}=D^{2}W(x_{0})[a,b]$. Define
\[
  L_{M}a=\Delta_{M}\T(W h-W_{a}\rs^{M})
\]
and
\[
  T_{M}(a,b)=
  h\T W k-h\T W_{b}\rs^{M}-k\T W_{a}\rs^{M}
  +\tfrac12(\rs^{M})\T W_{ab}\rs^{M}.
\]
Then the full profile Hessian is the bilinear form
\begin{equation}
  Q_{M}(a,b)=T_{M}(a,b)-(L_{M}a)\T H_{M}^{-1}(L_{M}b),
  \qquad H_{M}=A_{M}+C_{M}.
  \label{eq:profile-hessian-general}
\end{equation}
The nested statistic obeys
\[
  2N\{f_{B}(\hat x)-f_{A}(\hat x)\}
  \;\xrightarrow{d}\;
  Z\T (Q_{B}-Q_{A})Z,
\]
with reference spectrum
\[
  \lambda_{j}=\operatorname{eig}\{(Q_{B}-Q_{A})\Gamma_{x}\}.
\]
If the weight is fixed, $W_{a}=W_{ab}=0$, $x=u$, and
\begin{equation}
  Q_{M}=W-W\Delta_{M}H_{M}^{-1}\Delta_{M}\T W.
  \label{eq:profile-hessian-fixed}
\end{equation}
If $W=W(u)$, take $x=u$ and $P=I$. If the weight is estimated from additional
stage-one objects, such as a diagonal or dense NACOV estimate, those objects
must be included in $x$ and in the covariance of $Z$.
\end{proposition}

\begin{proof}
Taylor expand the two profiled scalar functions around $x_{0}$:
\[
  f_{M}(x_{0}+N^{-1/2}Z)
  =f_{M}(x_{0})+N^{-1/2}s_{M}(x_{0})\T Z
...
law. For the directional formula, differentiate the population first-order
condition $\Delta\T W\rs=0$ in direction $a$. The parameter derivative solves
\[
  H_{M}D\theta[a]=\Delta_{M}\T(W h-W_{a}\rs^{M})=L_{M}a.
\]
Thus
$D\rs^{M}[a]=\Delta_{M}H_{M}^{-1}L_{M}a-h$. The second derivative of the
unprofiled discrepancy with respect to the first-stage arguments, holding
$\theta$ fixed, is $T_{M}(a,b)$. The standard profile-Hessian identity
$f_{xx}-f_{x\theta}H^{-1}f_{\theta x}$ gives
\eqref{eq:profile-hessian-general}; the fixed-weight matrix form follows by
setting $W_{a}=W_{ab}=0$.
\end{proof}
...
useful diagnostic for the standard-error machinery, separate from the
profile-Hessian law above.
...
This identity has a narrow scope: $\Wr$ is linear in $\rs$, so the residual-vcov
deformation is quadratic in $\rs$ and vanishes when a model fits. The numerical
check confirms the identity to machine precision and separately computes the
profile-Hessian law.
...
channels, curvature and weight. The CFA check script compares the
residual-vcov quadratic with the finite-difference profile-Hessian law when
$C\neq0$.

\section{Next calculation}
\label{sec:next}

The next useful object is a prototype nested-test driver that estimates
$Q_{B}-Q_{A}$ directly from the profile score and compares the resulting mixture
with simulated values of $2N(f_{B}-f_{A})$. The clean first target is the
normal-theory weight $W_{\mathrm{NT}}(u)$, because $x=u$, $\Gamma_{x}=\Gamma$,
and the second derivative of $W_{\mathrm{NT}}$ is a closed-form matrix calculus
exercise. That target isolates the profile-Hessian issue without the additional
NACOV layer.

The second target is DWLS. There $x$ should be enlarged to carry the moment
vector and the diagonal NACOV estimator, for example $x=(u,\gamma)$ with
$W=\diag(\gamma)^{-1}$. The terms in \eqref{eq:profile-hessian-general} are then
simple in the weight block:
$DW[a]_{kk}=-a_{\gamma,k}/\gamma_{k}^{2}$ and
$D^{2}W[a,b]_{kk}=2a_{\gamma,k}b_{\gamma,k}/\gamma_{k}^{3}$. The hard part is
not the derivative of $W$; it is the joint covariance of $(u,\gamma)$ and the
choice of a misspecified nested null where the larger pseudo-true model still
lies in the smaller model. A good simulation should report three spectra side by
side: ordinary Satorra, profile-Hessian, and residual-vcov diagnostic. The
profile-Hessian spectrum is the one to compare with the empirical nested
statistic.
