...
\[
  df \quad\longrightarrow\quad \tr(U_M\Gamma),
\]
where $U_M$ is the first-order residual projector and $\Gamma$ is the asymptotic
...
\[
  ...
  + o(n^{-1}),
\]
with $Q_M$ built from the observed population Hessian, not just the
Gauss--Newton Hessian. For a fixed weight and an unbiased first-stage covariance
convention this reduces to $\tr(Q_M\Gamma)$.

The notation is deliberate. $U_M$ is reserved for the exact-fit or
Gauss--Newton residual projector. $Q_M$ denotes the profile Hessian of the
fitted value function. For an absolute-fit statistic, the comparator is the
saturated model, whose profile value is identically zero, so the quadratic form
is $Q_M-Q_{\mathrm{sat}}=Q_M$. For a nested test, the corresponding object is
$Q_B-Q_A$.
...
\[
  \E[\hat F]=F_0+n^{-1}\tr(Q_M\Gamma)+o(n^{-1}).
\]
Under correct specification, $e=0$, so $K=0$, $Q_M$ reduces to the
usual first-order residual projector
\[
  U_M = W-WD(D'WD)^{-1}D'W ,
\]
and the bias reduces to $n^{-1}\tr(U_M\Gamma)$. Under normal-theory weighting
...
automatically $df$ or $\tr(U_M\Gamma)$.
...
$\tr(Q_M\Gamma)$. This is the most direct target for simulation checks:
compare $n\{\E[\hat F]-F_0\}$ with both $\tr(U_M\Gamma)$ and
$\tr(Q_M\Gamma)$.
...
$\tr(Q_M\Gamma)$ rather than $\tr(U_M\Gamma)$ unless either the residual or the
model curvature is negligible. This is the same phenomenon as replacing expected
information by observed Hessian in misspecification-robust standard errors.
...
case. Once $\tr(Q_M\Gamma)$ is verified there, add controlled
