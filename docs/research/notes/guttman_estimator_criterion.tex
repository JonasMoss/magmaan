...
\section{Step 3: choose CFA coordinates for the common part}
...
by the composite regression.

The incidence matrix \(Z\) gives labelled composite axes, so the Guttman map
does not use an arbitrary rotation \(C\). It keeps those axes and only changes
the scale of each column. The CFA marker convention then determines the scale:
the marker loading for factor \(f\) should be one. Let
\[
  M=\diag(K^\star_{m_1,1},\ldots,K^\star_{m_k,k})
\]
be the diagonal matrix of the current marker coefficients. Equivalently, define
the marker-scaled surrogate factor vector
\[
  \eta_G=M U .
\]
Then \(U=M^{-1}\eta_G\), so the marker-scaled Guttman matrices are
...
this is the unique rescaling that satisfies the marker convention, provided all
marker coefficients are nonzero.
...
loading is one, and the diagonal matrix \(T=\Lambda_0\T Z\) is nonsingular. Then the
...
Guttman estimate is obtained from \(H\), \(Z\), and the marker rescaling above;
...
communality-filled covariance \(H\), and marker-rescales the result into CFA
