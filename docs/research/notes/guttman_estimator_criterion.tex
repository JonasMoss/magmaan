...
squares coefficient vector in the regression of \(Y_{H,i}\) on the standardized
fixed composites \(F\). The stacked problem in \eqref{eq:composite-loss} is just
...
\(L=HZQ\), \(P=QZ\T HZQ\), and \(Q=D^{-1/2}\) gives the unstandardized form and
...
First, if \(P\) is positive definite, use the ordinary Guttman map above.
Second, if \(P\) is positive semidefinite and singular, use the
...
\[
  \lim_{\lambda\downarrow0}L(P+\lambda I)^{-1}.
\]
...
The parameter split is the difficult part. A singular \(P\) means that the
...
Third, if \(P\) is indefinite or \eqref{eq:compat} fails, the quadratic
...
for example
\[
  K_{\lambda}=L(P+\lambda I)^{-1},
\]
with \(\lambda\) chosen to make the system positive definite, or by first
projecting \(P\) to a positive semidefinite matrix. This is useful engineering,
...
\item Form \(B=Z\T HZ\), \(D=\diag(B)\), \(Q=D^{-1/2}\),
  \(L=HZQ\), and \(P=QBQ\).
