...
\[
  U=Z\T Y_H .
\]
Their covariance and their covariance with the variables are
\[
  B=\Cov(U)=Z\T H Z,
  \qquad
  A=\Cov(Y_H,U)=HZ .
...
If \(B\) is positive definite, the unique minimizer of
\eqref{eq:composite-loss} is
\begin{equation}\label{eq:Kstar}
  K^\star = A B^{-1}=HZ(Z\T HZ)^{-1}.
\end{equation}
...
\[
  ...
  -2A+2KB .
\]
The first-order condition is \(KB=A\), giving \(K^\star=AB^{-1}\). Since \(B\)
is positive definite, this stationary point is the unique minimum. Substituting
\(A=HZ\) and \(B=Z\T HZ\) gives \eqref{eq:common-proj}.
\end{proof}
...
Equations \eqref{eq:Kstar} and \eqref{eq:common-proj} use inverses, so the
interior construction is not automatically total. A boundary-complete version
should distinguish three cases.

First, if \(B\) is positive definite, use the ordinary Guttman map above.
Second, if \(B\) is positive semidefinite and singular, use the
...
\[
  \lim_{\lambda\downarrow0}A(B+\lambda I)^{-1}.
\]
...
The parameter split is the difficult part. A singular \(B\) means that the
fixed composite space has rank below \(k\). The common covariance still has a
canonical Moore-Penrose fitted value, but the marker-scaled
...
Third, if \(B\) is indefinite or \eqref{eq:compat} fails, the quadratic
...
for example
\[
  K_{\lambda}=A(B+\lambda I)^{-1},
\]
with \(\lambda\) chosen to make the system positive definite, or by first
projecting \(B\) to a positive semidefinite matrix. This is useful engineering,
but it should be reported as a regularized Guttman variant, not as the clean
population map.
...
\item Form \(A=HZ\) and \(B=Z\T HZ\).
\item Solve \(K^\star=AB^{-1}\). This is the coefficient matrix minimizing the
  composite prediction loss when \(B\) is positive definite.
\item Read marker coefficients into \(M\); compute
  \(\Lambda_G=K^\star M^{-1}\), \(\Phi_G=MBM\), and
  \(\psi_G=\diag(S-\Lambda_G\Phi_G\Lambda_G\T)\).
