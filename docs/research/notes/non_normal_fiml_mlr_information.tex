...
Under non-normality they are different estimators of different limiting
objects.

\begin{conclusion}
Structured H1 information is not 100\% mathematically correct for non-normal
FIML in the same way it is correct for normal FIML. It is correct only under
conditions that collapse the pseudo-true saturated curvature and the
model-implied structured curvature. Robust FIML theory by itself supplies the
sandwich \(H^{-1}JH^{-1}\), not the substitution
\(H_\eta\{\eta(\theta^\star)\}\) for \(H_\eta(\eta^\star)\).
\end{conclusion}
...
\section*{Disproof strategy}

To disprove universal correctness of structured H1 information, it is enough
to exhibit a non-normal distribution with the same missingness mechanism for
which
\[
  H_\eta\{\eta^\star\}
  \ne
...
ordinal-as-continuous examples already suggest this: \(S\) estimates the
covariance of integer-coded variables, while \(\Sigma(\hat\theta)\) estimates
the closest restricted SEM covariance. Unless the restricted model is
saturated, these limits differ. Missingness adds pattern-specific curvature,
so equality is even less automatic.
...
A maximally small complete-data example already shows the issue. Let the
saturated covariance of two observed variables be
\[
  \Sigma_Y =
  \begin{bmatrix}
    1 & \rho \\
...
This witness uses complete data only to keep the algebra visible. Missing data
replaces \(\Gamma_{\mathrm{NT}}(\Sigma_Y)\) by the saturated observed-pattern
sandwich
\[
  H_\eta^{-1}J_\eta H_\eta^{-1},
\]
