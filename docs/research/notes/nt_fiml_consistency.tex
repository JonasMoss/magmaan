...
With missing data the saturated MLE no longer has a closed form (it needs EM),
so we switch from Lemma~\ref{lem:sat} to the estimating-equation view. Let unit
$i$ have observed index set $o_i$ and contribute the Gaussian \emph{marginal}
log-density $\log\phi(x_{i,o_i};\mu_{o_i},\Sigma_{o_io_i})$ with score
$h_i(\eta)$.

\begin{theorem}[MCAR consistency, distribution-free]\label{thm:mcar}
Suppose missingness is MCAR, i.e.\ the response pattern $R$ is independent of
$X$, with finite second moments, and every entry of $(\mu_0,\Sigma_0)$ is
identified by some observed pattern. Then $\hat\eta\xrightarrow{p}\eta_0$; with a
correctly specified structure, $\hat\theta\xrightarrow{p}\theta_0$.
\end{theorem}

\begin{proof}
Fix a pattern $o$. Under MCAR the units that realize $o$ are a random subsample,
so $X_o$ carries the population marginal moments $(\mu_{0,o},\Sigma_{0,oo})$. The
Gaussian marginal score is an unbiased estimating equation for those moments:
its mean part has expectation
$\E[\Sigma_{0,oo}^{-1}(X_o-\mu_{0,o})]=0$, and its covariance part is unbiased
because $\E[(X_o-\mu_{0,o})(X_o-\mu_{0,o})']=\Sigma_{0,oo}$. Hence
$\E\{h_o(\eta_0)\}=0$ for each $o$, and averaging over the MCAR pattern
distribution preserves the zero. So $\eta_0$ is the population root of the
stacked estimating equation $\sum_i h_i(\eta)=0$; $Z$-estimator consistency,
under the stated identification, gives $\hat\eta\xrightarrow{p}\eta_0$, and the
argmin step of Theorem~\ref{thm:complete} carries it to $\hat\theta$.
\end{proof}

This is the consistency content behind the ``What MCAR buys'' section of the
companion information note: under MCAR the observed-pattern normal score is
unbiased at the full-data first two moments, no normality required.
