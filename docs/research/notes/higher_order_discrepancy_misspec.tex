...
ML vs GLS & differ (pseudo-true, $\beta_W$) & coincide to leading order\\
...
The three estimators share the quadratic-form weight at the data limit,
$V=\tfrac12 D'(\Sigma_0^{-1}\otimes\Sigma_0^{-1})D$ for ML and GLS and $V=\tfrac12
D'D$ for ULS, but they differ in where the weight is anchored, and this is what
separates their RMSEAs once the drift is gone.
...
\paragraph{ML.} The ML discrepancy is not a fixed quadratic; its estimating
equation is the reweighted normal equation
$\dot\sigma'V(\sigma(\theta))\,(\sigma_0-\sigma(\theta))=0$ with the weight
anchored at the \emph{fitted model} $\Sigma(\theta)$. Two consequences. First,
the pseudo-true value solves a model-metric projection, so under fixed
misspecification $\theta_\ast^{\mathrm{ML}}\ne\theta_\ast^{\mathrm{GLS}}$ and
$F_0^{\mathrm{ML}}\ne F_0^{\mathrm{GLS}}$: ML-RMSEA and GLS-RMSEA estimate
\emph{different population numbers}, already at zeroth order, because they measure
misfit in different metrics (Olsson, Foss, Troye and Howell
\cite{olssonetal2000}). Second, the $\theta$-dependence of the weight adds a
discrepancy-curvature term $K_F$ to $B$ (a third derivative of $F_{\mathrm{ML}}$),
and routes the weight fluctuation through the projected increment
$\dot\sigma(\hat\theta-\theta_\ast)$ rather than the raw $z$. All three effects
collapse under correct specification ($\sigma(\theta_\ast)=\sigma_0$, $e=0$),
where ML, GLS and ULS coincide to the order that matters.
...
For ULS the weight is fixed, so $\beta_W\equiv0$ and Corollary~\ref{cor:bias}
predicts $N\,(\E[\hat F]-F_0)\to\tr(\tilde U\Gamma)$ exactly, isolating the new
object $\tilde U$. A one-factor model is fit to data from a two-factor population
(six indicators, three per factor, factor correlation $0.55$), a misspecification
the one-factor model cannot absorb: $df=9$, $F_0=0.231$, $\|e\|=0.48$, curvature
coupling $\|K\|=0.68$. The table reports $N$ times the Monte-Carlo bias of
$\hat F$ against the naive $\tr(U\Gamma)$ (Gauss--Newton $A$) and the
curvature-corrected $\tr(\tilde U\Gamma)$ (Hessian $B$). The sample covariance is
the unbiased $\div(N{-}1)$ form; the $\div N$ form carries its own $O(1/N)$
moment bias which contaminates exactly this order. Under normality
$\Gamma=\Gamma_{\mathrm{NT}}$ is closed form; under $t_6$ data $\Gamma$ is a
...
\hline
normal & $200$ & $2.20\ (0.05)$ & $3.38$ & $\mathbf{2.26}$\\
normal & $800$ & $2.24\ (0.10)$ & $3.38$ & $\mathbf{2.26}$\\
$t_6$  & $200$ & $4.26\ (0.08)$ & $7.00$ & $\mathbf{4.74}$\\
$t_6$  & $800$ & $4.60\ (0.16)$ & $7.00$ & $\mathbf{4.74}$\\
\end{tabular}
\end{center}

Every cell matches the curvature-corrected trace and rejects the naive one: at
$N=800$ the empirical bias sits within one Monte-Carlo standard error of
$\tr(\tilde U\Gamma)$ in both data conditions, while $\tr(U\Gamma)$ is wrong by
the full gap (more than twenty standard errors at $N=200$, normal). The small
$N=200$ shortfall in the $t_6$ cell is the expected $O(1/N)$ remainder, which
closes by $N=800$. The curvature term is not a third-decimal refinement: it is a
third to a half of the entire bias. Script:
...
least relevant and the near-null device should take over. The ML term $K_F$ and
the projected weight fluctuation are stated, not derived; the full ML expansion
needs the third derivative of $F_{\mathrm{ML}}$ and is left for the numerical
companion. The expansion assumes finite fourth moments for $\Gamma$ and finite
sixth moments for the $n^{-1}$ bias to have finite expectation; under heavier
tails \eqref{eq:bias} is a stochastic-order statement, not a moment statement.
\end{remark}
