...
binary <- file.path(project_root, "build", args$preset, "benchmarks",
                    "magmaan_ordinal_workspace_bench")
workspace_csv <- file.path(results, "workspace.csv")
...
if (!isTRUE(args$skip_build)) {
  ...
  run_cmd("cmake", c("--build", "--preset", args$preset,
                     "--target", "magmaan_ordinal_workspace_bench"),
          project_root)
}
...
write_metadata(
  metadata_csv,
  values = list(
    experiment = "06-ordinal-snlls-probe",
    question = paste(
      "Compare materialized bounded, cache-aware bounded, and cache-aware",
      "SNLLS fits for all-ordinal delta LS."
    ),
    preset = args$preset,
    smoke = args$smoke,
    reps = args$reps,
