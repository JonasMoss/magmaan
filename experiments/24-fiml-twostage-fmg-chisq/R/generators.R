# Non-normal marginal generators (single group).
#
...
# to the population covariance, then the population mean is swept in. Marginal
# skew and kurtosis are central/standardized, so the mean shift leaves them
# unchanged. This is the single-group reduction of experiment 23's generators
# (leaves cannot source siblings, so the machinery is carried locally).
...
vm_setup <- function(Sigma, fl) {
  ...
  if (min(eigen(Rz, symmetric = TRUE, only.values = TRUE)$values) <= 0) {
    stop("VM: intermediate correlation matrix not PD", call. = FALSE)
  }
...
# the complete single-group data frame for replicate i (the caller injects
# missingness). norm/VM draw fresh per replicate from a per-replicate seed;
# IG/PL calibrate once and pre-draw a reps-sized batch indexed by replicate.
build_cell_sampler <- function(pop, dist, n, reps, seed_base,
  ...
  if (fam == "norm") {
    ...
    draw <- function(i) {
      ...
      .finish_block(matrix(stats::rnorm(n * p), n, p) %*% L, m$mu, pop$ov)
    }
  } else if (fam == "vm") {
    fl    <- fleishman_coef(mom[[1L]], mom[[2L]])
    setup <- vm_setup(m$Sigma, fl)
    draw <- function(i) {
      set.seed(seed_base + i)
      .finish_block(vm_draw_block(n, p, setup, fl), m$mu, pop$ov)
    }
  } else if (fam == "ig") {
    ...
    batch <- core$sim_ig_draw(cal, n = n, reps = reps, seed_base = seed_base,
                              quadrature_points = knobs$ig_quadrature_points)
    draw <- function(i) .finish_block(batch$draws[[i]], m$mu, pop$ov)
  } else if (fam == "pl") {
    if (is.null(core)) stop("PL cells require magmaan_core", call. = FALSE)
    sds <- sqrt(diag(m$Sigma))
    ...
    draw <- function(i)
      .finish_block(sweep(batch$draws[[i]], 2L, sds, "*"), m$mu, pop$ov)
  } else {
