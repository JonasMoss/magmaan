...
# Overnight RMSE / coverage / timing study: the chosen closed-form recipe
# (extended_triad_ls communality + standardized composite on the aligned
# configural Guttman map) versus NTML. Two arms only. Categorical cells use
# magmaan's ordinal Pearson-code simulator and fit the code covariance directly;
# no polychoric statistics enter either arm. Question: does the closed-form
# recipe match NTML on parameter RMSE and empirical-SE coverage -- including the
# high-rho / weak-loading regime that broke the retired adaptive composite -- at
# a modest implied-Sigma-fit cost, while being faster and never failing to fit?
...
parse_args <- function(args) {
  ...
  apply_mode <- function(mode) {
    ...
    if (mode == "probe") {
      ...
      # Two-arm overnight grid: the chosen recipe (extended_triad_ls +
      # standardized, aligned configural) vs NTML. n = 50 stresses small-sample
      # coverage; strength = weak is the regime where the ratio-of-ratios
      # communalities hurt; rho spans orthogonal -> strong (0.8 is the ex-bad-boi
      # regime, now benign under the standardized composite); ig + ordinal test
      # the empirical-SE coverage under non-normality and category scores.
...
# The chosen estimator: extended_triad_ls communality + standardized composite on
# the aligned configural map. The configural entry does not yet expose communality
# selection, so this runs through the restricted-map entry with an unconstrained
# partable -- a bit-exact and (post speedup) fast proxy for the configural aligned
# map with a selectable communality.
...
run_arm <- function(cond, rep_id, arm, pt, ss, X, pop, info, target) {
  ...
  fit_expr <- switch(
    arm,
    guttman = quote(guttman_fit(pt, ss)),
...
for (cell in seq_len(nrow(grid))) {
  ...
  append_csv(summarise_estimates_joint(estimate_draws, baseline), "estimate_joint_summary", opts$results_dir)
  append_csv(summarise_whole(whole_draws), "whole_summary", opts$results_dir)
  append_csv(summarise_whole_joint(whole_draws, baseline), "whole_joint_summary", opts$results_dir)
