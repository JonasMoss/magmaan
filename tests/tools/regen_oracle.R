...
# Shared lavaan-to-JSON fixture helpers, also used by regen_parity_fixtures.R.
...
ordinal_cases <- list(
  ...
  list(id = "0010_near_perfect_pair",
       ...
       group = NULL,
       fit = FALSE),
  # Threshold-heavy boundary: 6 categories per variable (5 thresholds each,
  # 20 thresholds total) with deliberately sparse extreme cells. Stresses the
  # threshold-count machinery and pairwise polychoric tables under thin tails.
  list(id = "0011_sixcat_threshold_heavy_cfa",
       model = ordinal_model_4,
       data = make_ord_df(720, list(c(-1.85, -0.75, -0.05, 0.65, 1.80),
                                    c(-1.70, -0.60, 0.05, 0.70, 1.75),
                                    c(-1.95, -0.85, -0.10, 0.60, 1.90),
                                    c(-1.60, -0.55, 0.15, 0.80, 1.70)),
                          seed = 131L),
       ordered = paste0("x", 1:4),
       group = NULL,
       fit = TRUE),
...
#   * absent cross-loading/covariance rows generated by modindices()
#   * one-dimensional equality releases from `lavTestScore()`
#
# Structural-regression absent rows are deliberately out of scope for magmaan's
# modification-index enumeration until the Reduced-LISREL row contract settles.
