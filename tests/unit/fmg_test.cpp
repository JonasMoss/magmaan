...
TEST_CASE("FMG standard chi-square ignores the UGamma spectrum") {
  ...
  const auto r = magmaan::robust::fmg_test(
      7.0, 4, eig,
      magmaan::robust::FmgOptions{
          .method = magmaan::robust::FmgMethod::StandardChiSquare});
...
TEST_CASE("FMG selects top df eigenvalues and truncates negative noise") {
  ...
  const auto r = magmaan::robust::fmg_test(
      3.0, 3, eig,
      magmaan::robust::FmgOptions{
          .method = magmaan::robust::FmgMethod::All});
...
TEST_CASE("FMG exact and classical approximations collapse for equal lambdas") {
  ...
  for (const auto method : {
           magmaan::robust::FmgMethod::All,
           magmaan::robust::FmgMethod::SatorraBentler,
           magmaan::robust::FmgMethod::ScaledShifted,
           magmaan::robust::FmgMethod::ScaledF}) {
    const auto r = magmaan::robust::fmg_test(
        chi2, 4, eig, magmaan::robust::FmgOptions{.method = method});
...
TEST_CASE("FMG PALL shrinks every eigenvalue halfway to the mean") {
  ...
  const auto r = magmaan::robust::fmg_test(
      4.0, 3, eig,
      magmaan::robust::FmgOptions{
          .method = magmaan::robust::FmgMethod::PenalizedAll});
...
TEST_CASE("FMG pEBA uses lavaan/semTests column-block averaging") {
  ...
  const auto r = magmaan::robust::fmg_test(
      6.0, 5, eig,
      magmaan::robust::FmgOptions{
          .method = magmaan::robust::FmgMethod::Peba,
          .param = 2.0});
...
TEST_CASE("FMG pOLS shrinks the spectrum trend by gamma") {
  ...
  const auto r = magmaan::robust::fmg_test(
      6.0, 4, eig,
      magmaan::robust::FmgOptions{
          .method = magmaan::robust::FmgMethod::Pols,
          .param = 2.0});
...
TEST_CASE("FMG reduced-matrix entry point eigensolves before p-value") {
  ...
  auto r_or = magmaan::robust::fmg_test_from_reduced_matrix(
      4.0, 2, M,
      magmaan::robust::FmgOptions{
          .method = magmaan::robust::FmgMethod::PenalizedAll});
...
TEST_CASE("FMG nested wrapper rejects single-model approximations") {
  ...
  auto r_or = magmaan::robust::lr_test_fmg(
      4.0, sd,
      magmaan::robust::FmgOptions{
          .method = magmaan::robust::FmgMethod::SatorraBentler});
