...
#include "magmaan/fit/inference.hpp"     // chi2_pvalue (oracle for p-values)
#include "magmaan/fit/lr_test_satorra.hpp"
#include "magmaan/fit/raw_data.hpp"      // empirical_gamma (oracle Γ̂)
...
Eigen::VectorXd naive_satorra_eigvals(
    const std::vector<lf::SatorraGroup>& groups,
  ...
  auto Gamma_or = lf::empirical_gamma(gr.X);
...
TEST_CASE("compute_satorra2000: 2-var saturated, σ₁₂=0 restriction") {
  ...
  auto fast_or = lf::compute_satorra2000({gr}, A_alpha,
                                          lf::GammaSource::Empirical);
...
TEST_CASE("compute_satorra2000: NT-Γ sanity (all eigvals → 1)") {
  ...
  lf::SatorraGroup gr{Pi_alpha, S, X, mean, 1.0,
                      static_cast<std::int32_t>(n)};
  auto r = lf::compute_satorra2000({gr}, A_alpha, lf::GammaSource::NT);
  REQUIRE(r.has_value());
...
TEST_CASE("compute_satorra2000: degenerate m=0 (H0 ≡ H1)") {
  ...
  lf::SatorraGroup gr{Eigen::MatrixXd::Identity(3, 3), S, X, mean, 1.0,
                      static_cast<std::int32_t>(n)};
  const Eigen::MatrixXd A_alpha = Eigen::MatrixXd::Zero(0, 3);
  auto r = lf::compute_satorra2000({gr}, A_alpha);
  REQUIRE(r.has_value());
  CHECK(r->C.size() == 0);
  CHECK(r->S.size() == 0);
...
TEST_CASE("lr_test_satorra2000: low-level p-value wrap") {
  ...
  const double p_adj_expected =
      1.0 - lf::noncentral_chisq_cdf(Tadj_expected, d0_expected, 0.0);
  CHECK(r.p_adjusted == doctest::Approx(p_adj_expected));

  // Mixture p-value matches `imhof_upper` directly.
  CHECK(r.p_mixture == doctest::Approx(lf::imhof_upper(eig, T_diff)));
}
...
TEST_CASE("lr_test_satorra2000: degenerate (m = 0)") {
  ...
  auto r_or = lf::lr_test_satorra2000(0.0, sd);
  REQUIRE(r_or.has_value());
  const auto& r = *r_or;
  CHECK(r.df_diff == 0);
