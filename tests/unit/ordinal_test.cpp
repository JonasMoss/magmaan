...
TEST_CASE("Polyserial pair h-weighted kernel downweights continuous-tail discordance") {
  ...
  auto cont_ml = magmaan::data::fit_polyserial_pair_rho_ml(
      cat_cont, u_cont, th);
  auto ml_like = magmaan::data::fit_polyserial_pair_rho_h_weighted(
      cat_cont, u_cont, th,
      magmaan::data::PolyserialPairHWeightedOptions{
          .h_score = magmaan::data::PolychoricHScoreOptions{
              .kind = magmaan::data::PolychoricHScoreKind::ML}});
  auto robust = magmaan::data::fit_polyserial_pair_rho_h_weighted(
      cat_cont, u_cont, th,
      magmaan::data::PolyserialPairHWeightedOptions{
          .h_score = magmaan::data::PolychoricHScoreOptions{
  ...
  REQUIRE(clean_ml.has_value());
  REQUIRE(cont_ml.has_value());
  REQUIRE(ml_like.has_value());
  REQUIRE(robust.has_value());
  CHECK(ml_like->rho == doctest::Approx(cont_ml->rho).epsilon(1e-10));
  CHECK(cont_ml->rho < clean_ml->rho);
  CHECK(robust->rho > cont_ml->rho);
  CHECK(std::abs(robust->rho - clean_ml->rho) <
        std::abs(cont_ml->rho - clean_ml->rho));
  CHECK(robust->weights.tail(n_bad).maxCoeff() <
        robust->weights.head(n_clean).mean());
  CHECK(robust->ratios.tail(n_bad).minCoeff() >
        robust->ratios.head(n_clean).mean());
  CHECK(robust->probabilities.size() == cat_cont.size());
  CHECK(robust->joint_densities.size() == cat_cont.size());
  CHECK(robust->ratios.size() == cat_cont.size());
  CHECK(robust->weights.size() == cat_cont.size());
  CHECK(robust->weights.allFinite());
}
...
TEST_CASE("Mixed ordinal h-weighted polyserial stats preserve marginals and robustify mixed pairs") {
  ...
  std::vector<std::vector<std::int32_t>> ordered = {{1, 1, 0, 0}};
  auto ml = magmaan::data::mixed_ordinal_stats_from_data({X}, ordered);
  auto robust = magmaan::data::mixed_ordinal_stats_polyserial_h_weighted_from_data(
      {X}, ordered,
      magmaan::data::MixedOrdinalPolyserialHWeightedStatsOptions{
          .polyserial = magmaan::data::PolyserialPairHWeightedOptions{
              .h_score = magmaan::data::PolychoricHScoreOptions{
                  .kind = magmaan::data::PolychoricHScoreKind::WmaHardCap,
                  .k = 2.0}}});
  REQUIRE(ml.has_value());
  ...
  CHECK(robust->thresholds[0].isApprox(ml->thresholds[0], 0.0));
  CHECK(robust->mean[0].isApprox(ml->mean[0], 0.0));
  CHECK(robust->R[0](0, 1) == doctest::Approx(ml->R[0](0, 1)));
  CHECK(robust->R[0](2, 3) == doctest::Approx(ml->R[0](2, 3)));
  CHECK(std::abs(robust->R[0](2, 0) - ml->R[0](2, 0)) > 1e-4);
  CHECK(robust->moments[0].size() == ml->moments[0].size());
  CHECK(robust->NACOV[0].rows() == ml->NACOV[0].rows());
  CHECK(robust->NACOV[0].allFinite());
  CHECK(robust->W_dwls[0].diagonal().minCoeff() > 0.0);
}
