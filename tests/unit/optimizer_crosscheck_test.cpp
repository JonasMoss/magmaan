...
//
//   • NLopt L-BFGS — line search (the current default; validated against
//                    lavaan by the parity golden tests).
...
//   • NLopt SLSQP  — sequential quadratic programming.
//
// NLopt L-BFGS is the reference because the lavaan-parity goldens already pin
// it to lavaan's nlminb result; if SLSQP and Port agree with NLopt L-BFGS, they agree
...
TEST_CASE("optimizer cross-check: PORT drmngb matches NLopt L-BFGS on ML") {
  ...
  auto est_nlopt = magmaan::test::fit(*pt, *mr, samp, Bounds{}, Backend::NloptLbfgs);
  auto est_port  = magmaan::test::fit(*pt, *mr, samp, Bounds{}, Backend::Port);
  REQUIRE(est_nlopt.has_value());
  REQUIRE(est_port.has_value());

  CHECK(est_nlopt->fmin > 0.0);
  CHECK(est_port->fmin ==
        doctest::Approx(est_nlopt->fmin).epsilon(1e-3));
  CHECK((est_port->theta - est_nlopt->theta).cwiseAbs().maxCoeff() < 5e-3);
}
#endif  // MAGMAAN_WITH_PORT

#ifdef MAGMAAN_WITH_PORT
TEST_CASE("optimizer cross-check: PORT NL2SOL matches NLopt L-BFGS on a feasible ULS fit") {
  ...
  // optimum, which is fine for NLopt L-BFGS / Port / SLSQP — but PortNls's
  ...
  auto est_nlopt = magmaan::estimate::fit_gmm(
      *pt, *mr, samp, *x0, {}, {}, Backend::NloptLbfgs);
  ...
  CHECK(est_port_nls->fmin ==
        doctest::Approx(est_nlopt->fmin).epsilon(1e-4));
  CHECK((est_port_nls->theta - est_nlopt->theta).cwiseAbs().maxCoeff() < 5e-3);
}
#endif  // MAGMAAN_WITH_PORT
...
TEST_CASE("optimizer cross-check: NLopt gradient backends match NLopt L-BFGS on ML") {
  // Three new gradient-based NLopt algorithms (TNEWTON, VAR2, NLopt's own
  // L-BFGS) all on the same unbounded ML problem. SLSQP is the existing
  ...
  for (auto backend : {Backend::NloptSlsqp, Backend::NloptTnewton,
    ...
    CHECK(est->fmin == doctest::Approx(est_nlopt->fmin).epsilon(1e-3));
    CHECK((est->theta - est_nlopt->theta).cwiseAbs().maxCoeff() < 5e-3);
  }
}
