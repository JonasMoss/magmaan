...
TEST_CASE("parser flat fixtures match lavaan") {
  ...
  const std::string flat_dir = latva::test::fixtures_dir() + "/flat";
  ...
  for (const auto& e : corpus) {
    ...
    auto got = latva::parse::Parser::parse(e.model);
    ...
    nlohmann::json got_json =
        latva::test::flat_partable_to_json(e.model, e.id, *got);

    auto a = latva::test::strip_meta(got_json);
    auto b = latva::test::strip_meta(exp);
    if (a != b) {
