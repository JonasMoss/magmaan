...
using magmaan::test::CorpusEntry;
using magmaan::test::LexerScan;
...
// (when MAGMAAN_REGEN_FIXTURES is set in the environment).
//
...
TEST_CASE("lexer goldens — every corpus entry round-trips") {
  ...
  const std::string lexer_dir = magmaan::test::fixtures_dir() + "/lexer";
  const bool regen = magmaan::test::regen_mode();

  for (const CorpusEntry& e : corpus) {
    CAPTURE(e.id);
    const std::string path = lexer_dir + "/" + e.id + ".tokens.json";

    LexerScan got = magmaan::test::scan_all(e.model);
    std::optional<magmaan::ParseError> err = got.error;
    const auto fixture =
        magmaan::test::format_lexer_fixture(e.id, e.model, got.tokens, err);

    if (regen) {
      const bool ok = magmaan::test::write_fixture(path, fixture);
      REQUIRE(ok);
    ...
    REQUIRE_MESSAGE(raw.has_value(),
                    ...
                    << " (run with MAGMAAN_REGEN_FIXTURES=1 to bootstrap)");
    ...
    const std::string diff =
        magmaan::test::diff_lexer_fixture(e.id, e.model, got, j);
    if (!diff.empty()) {
      FAIL(diff);
    }
  }
...
TEST_CASE("lexer negatives — every input produces the expected error kind") {
  const auto cases = magmaan::test::load_lexer_negatives();
  REQUIRE(!cases.empty());

  const std::string lexer_dir = magmaan::test::fixtures_dir() + "/lexer/negative";
  const bool regen = magmaan::test::regen_mode();

  for (const auto& c : cases) {
    CAPTURE(c.id);
    LexerScan got = magmaan::test::scan_all(c.input);
    ...
    const auto fixture =
        magmaan::test::format_lexer_fixture(c.id, c.input, got.tokens, got.error);
    ...
    const std::string diff =
        magmaan::test::diff_lexer_fixture(c.id, c.input, got, j);
    if (!diff.empty()) {
      FAIL(diff);
    }
  }
