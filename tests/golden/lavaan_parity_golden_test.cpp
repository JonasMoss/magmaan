...
// table (FIML/LS/ordinal lack a standard-SE or fit-measure path in magmaan,
...
TEST_CASE("lavaan-parity FIML — bfi missing=ml") {
  ...
  const bool aligned = ref.value("magmaan_aligned", false);
  if (!aligned || !est_or.has_value()) {
    notes.push_back(est_or.has_value()
        ? id + ": KNOWN GAP (magmaan_aligned=false)"
        : id + ": KNOWN GAP — fit_fiml failed: " + est_or.error().detail);
    for (const auto& n : notes) MESSAGE(n);
    CHECK(aligned);            // record the gap; not a hard failure of parity
    return;
  }
  const auto& est = *est_or;
  bool ok = true;
