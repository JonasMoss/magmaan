...
TEST_CASE("multi-group goldens — θ̂ / SE / df match lavaan") {
  ...
  for (const auto& e : corpus) {
    ...
    if (exp.contains("rls_chi2") && !exp["rls_chi2"].is_null()) {
      ...
      auto rls = magmaan::fit::rls_chi2(samp, im);
      if (!rls.has_value()) {
    ...
    if (exp.contains("browne_residual_nt") &&
      ...
      if (!skip_browne) {
        auto br = magmaan::fit::browne_residual_nt(*pt, *mr, samp, est);
        if (!br.has_value()) {
